"""Mock chat interface for testing device interactions."""
...
class MockPage(BasePage):
    """Mock chat interface for testing device-specific messaging."""
    ...
    def _build_header(self):
        ...
        self.name_label = tk.Label(header, text="Mock Chat", bg=Colors.SURFACE_HEADER, fg=Colors.TEXT_PRIMARY,
    ...
    def _setup_chat_history(self):
        for widget in self.history_frame.winfo_children():
            widget.destroy()
        self._message_counter = 0
        self._add_message("System", "Welcome to Mock Chat! This interface simulates device-to-device communication.", is_system=True)
    ...
    def _send_message(self, event=None):
        ...
        try:
            # For mock interface, simulate sending through mock transport
            if hasattr(self.controller, 'transport') and getattr(self.controller.transport, 'is_mock', False):
                self.controller.send_message(message)
            else:
                # Fallback: just add to local history for testing
                pass
        except Exception as e:
            print(f"Mock send error: {e}")
            return "break"

        # Add the sent message to our own history
        self._add_message(self._get_local_device_name(), message)
        self.msg_var.set("")

        # If connected to Mock device, also add a simulated response
        if (hasattr(self.session, 'device_name') and
            self.session.device_name and
            "mock" in self.session.device_name.lower()):
            # Simulate immediate response from Mock device
            self.after(500, lambda: self._add_simulated_mock_response(message))

        return "break"

    def _add_simulated_mock_response(self, original_message: str):
        """Add a simulated response from the Mock device."""
        responses = {
            "hello": "Mock device online. Ready for testing protocols.",
            "hi": "Mock device online. Ready for testing protocols.",
            "status": "Mock status: Connected, RSSI=-25dBm, Battery=100%",
            "test": f"Test message received. Echoing back: {original_message}",
            "ping": "Pong! Mock device responding.",
        }
        ...
        # Default response
        if response is None:
            response = f"Mock received: '{original_message}' (length: {len(original_message)} chars)"

        self._add_message("Mock Device", response)
    ...
    def sync_session_info(self):
        contact = self.session.device_name or "Mock Conversation"
        self.name_label.configure(text=f"Mock Chat - {contact}")
